...
<question pythoncode>
csq_interface = 'ace'
csq_prompt = "Write a function that takes in an index $i$ and returns the index of its parent. If given the root node, the function should return the root node index (0) again, for reasons we will see later."
