...
<showhide>
Here is one possible construction. (Assume each edge $uv$ in this explanation is directed, but in an undirected graph, we can just repeat with edge $vu$.) Let the $4$ copies of each vertex $v$ be $v_0,v_1,v_2,v_3$.<br><br>Each black edge $uv$ changes nothing about our state, so we include edges $u_0v_0,u_1v_1,u_2v_2,u_3v_3$. If we look at the index as a 2-bit binary number, we'll say that using a red edge flicks on the lower-order bit of the index, while using a blue edge flicks on the higher-order bit. For each red edge $uv$, we include edges $u_0v_1,u_1v_1,u_2v_3,u_3v_3$. Similarly, for each blue edge $uv$, we include edges $u_0v_2,u_2v_2,u_1v_3,u_3v_3$. Thus, getting from $s_0$ to $t_3$ requires flicking on both bits and thus forces the path to take a red and a blue edge.<br><br>Because we have $\Theta(1)$ copies of each vertex and edge, our runtime is still $\Theta(V+E)$ as before.
</showhide>
</checkyourself>